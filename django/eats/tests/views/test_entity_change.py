...
from eats.constants import FORWARD_RELATIONSHIP_MARKER, REVERSE_RELATIONSHIP_MARKER
...
class EntityChangeViewTestCase(ViewTestCase):
    ...
    def test_authentication(self):
        ...
        url = reverse("eats-entity-change", kwargs={"entity_id": entity.get_id()})
    ...
    def test_empty_entity(self):
        ...
        self.assertEqual(
            formset.initial_form_count(), 1, "Expected one pre-filled existence form"
        )
        ...
        self.assertEqual(existence_data["authority"], existence.authority.get_id())
        for formset in (
            "entity_type_formset",
            "entity_relationship_formset",
            "name_formset",
            "note_formset",
        ):
            self.assertEqual(
                response.context[formset].initial_form_count(),
                0,
    ...
    def test_post_redirect_add(self):
        ...
        self.assertEqual(
            response.request.url[len(response.request.host_url) :], add_url
        )

    def test_post_entity_relationships(self):
        entity = self.tm.create_entity(self.authority)
        entity2 = self.tm.create_entity(self.authority)
        rel_type = self.create_entity_relationship_type("is child of", "is parent of")
        ...
        self.assertEqual(response.request.url[len(response.request.host_url) :], url)
        ...
        self.assertEqual(form_data["related_entity"], assertion.range_entity.get_id())
        self.assertEqual(form_data["certainty"], True)
        self.assertEqual(assertion.certainty, self.tm.property_assertion_full_certainty)
        ...
        self.assertEqual(form_data["related_entity"], assertion.domain_entity.get_id())
        self.assertEqual(form_data["certainty"], False)
        self.assertEqual(assertion.certainty, self.tm.property_assertion_no_certainty)
    ...
    def test_post_entity_types(self):
        ...
        self.assertEqual(
            formset.initial_form_count(), 1, "Expected one pre-filled entity type form"
        )
        ...
        self.assertEqual(form_data["entity_type"], assertion.entity_type.get_id())
    ...
    def test_post_names(self):
        ...
        self.assertEqual(
            formset.initial_form_count(), 1, "Expected one pre-filled name form"
        )
    ...
    def test_post_name_part_delete(self):
        ...
        name_pa.name.create_name_part(name_part_type, language, script, "Carrie", 1)
        self.assertEqual(Name.objects.count(), 1)
        self.assertEqual(NamePart.objects.count(), 1)
        url = reverse("eats-entity-change", kwargs={"entity_id": entity.get_id()})
        form = self.app.get(url, user="user").forms["entity-change-form"]
    ...
    def test_post_notes(self):
        ...
        self.assertEqual(
            formset.initial_form_count(), 1, "Expected one pre-filled note form"
        )
    ...
    def test_post_subject_identifiers(self):
        ...
        url = reverse("eats-entity-change", kwargs={"entity_id": entity.get_id()})
        form = self.app.get(url, user="user").forms["entity-change-form"]
        form["subject_identifiers-1-subject_identifier"] = "http://www.example.org/test"
        ...
        self.assertEqual(form_data["assertion"], assertion.get_id())
        self.assertEqual(assertion.authority.get_id(), self.authority_id)
        self.assertEqual(form_data["subject_identifier"], assertion.subject_identifier)
        self.assertEqual(form_data["subject_identifier"], "http://www.example.org/test")
        ...
        self.assertEqual(form_data["subject_identifier"], assertion.subject_identifier)
        self.assertEqual(
            form_data["subject_identifier"], "http://www.example.org/test2"
        )
        ...
        self.assertEqual(form_data["subject_identifier"], assertion.subject_identifier)
        self.assertEqual(
            form_data["subject_identifier"], "http://www.example.org/test3"
        )
    ...
    def test_post_subject_identifiers_illegal(self):
        ...
        url = reverse("eats-entity-change", kwargs={"entity_id": entity1.get_id()})
        form = self.app.get(url, user="user").forms["entity-change-form"]
        form["subject_identifiers-0-subject_identifier"] = "http://www.example.org/test"
        form["subject_identifiers-1-subject_identifier"] = "http://www.example.org/test"
