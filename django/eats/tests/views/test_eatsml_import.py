...
class EATSMLImportViewTestCase(ViewTestCase):
    ...
    def test_authentication(self):
        ...
        non_editor = self.create_django_user("user2", "user2@example.org", "password")
    ...
    <authority xml:id="authority-1">
      <name>New authority</name>
        ...
        response = self.app.post(
            self.url, data, upload_files=[upload_file], user="user"
        ).follow()
        ...
        view_url = reverse(
            "eats-display-eatsml-import", kwargs={"import_id": eatsml_import.id}
        )
        self.assertEqual(
            response.request.url[len(response.request.host_url) :], view_url
        )
  ...
  <authorities>
    ...
    <authority>
      <name>Test</name>
        ...
        data = {"csrfmiddlewaretoken": csrf_token, "description": description}
        self.app.post(
            self.url, data, status=500, upload_files=[upload_file], user="user"
        )
        self.assertEqual(EATSMLImport.objects.count(), 0)
