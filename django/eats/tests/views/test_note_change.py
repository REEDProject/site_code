...
class NoteChangeViewTestCase(ViewTestCase):
    ...
    def test_non_matching_pa_note_change(self):
        ...
        self.app.get(
            reverse("eats-pa-note-change", kwargs=url_args), status=404, user="user"
        )
    ...
    def test_non_matching_date_note_change(self):
        ...
        url_args = {"entity_id": 0, "assertion_id": 0, "date_id": 0, "note_id": 0}
        self.app.get(
            reverse("eats-date-note-change", kwargs=url_args), status=404, user="user"
        )
        # Test with only the entity existing.
        entity = self.tm.create_entity(self.authority)
        url_args["entity_id"] = entity.get_id()
