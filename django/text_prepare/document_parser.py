...
class DocumentParser:
    ...
    def _define_grammar(self):
        ...
        content.set_whitespace_chars("")
        content.set_default_whitespace_chars("")
        white = pp.Word(" " + "\n")
        punctuation_chars = ". , ; : ' \" ( ) * / # $ % + - ? – _ ="
        punctuation = pp.oneOf(punctuation_chars)
        rich_content = pp.Word(pp.alphanums + " \\\n&[]" + punctuation_chars)
        rich_content.set_whitespace_chars("")
        ...
        # the text content.
        xml_escape = pp.oneOf("& >") | (
            pp.Literal("<") + pp.FollowedBy(white | ignored | integer)
        )
        ...
        damaged_code = (
            pp.Literal("<") + (pp.Word(".", min=1) ^ pp.Literal("…")) + pp.Literal(">")
        )
        ...
        exclamation_code = pp.Literal("@!").setParseAction(self._pa_exclamation)
        ...
        single_codes = (
            ...
            ^ semicolon_code
            ^ special_v_code
            ^ square_bracket_close_code
            ^ square_bracket_open_code
            ^ thorn_code
            ^ THORN_code
            ^ tilde_code
            ^ umlaut_code
        ...
        lang_ancient_greek_code = pp.nestedExpr("@grc\\", "@grc/", content=enclosed)
        lang_ancient_greek_code.setParseAction(self._pa_lang_ancient_greek)
        lang_anglo_norman_code = pp.nestedExpr("@xno\\", "@xno/", content=enclosed)
        ...
        lang_middle_cornish_code = pp.nestedExpr("@cnx\\", "@cnx/", content=enclosed)
        lang_middle_cornish_code.setParseAction(self._pa_lang_middle_cornish)
        lang_middle_high_german_code = pp.nestedExpr(
            "@gmh\\", "@gmh/", content=enclosed
        )
        lang_middle_high_german_code.setParseAction(self._pa_lang_middle_high_german)
        lang_middle_low_german_code = pp.nestedExpr("@gml\\", "@gml/", content=enclosed)
        lang_middle_low_german_code.setParseAction(self._pa_lang_middle_low_german)
        lang_middle_welsh_code = pp.nestedExpr("@wlm\\", "@wlm/", content=enclosed)
        lang_middle_welsh_code.setParseAction(self._pa_lang_middle_welsh)
        lang_old_english_code = pp.nestedExpr("@ang\\", "@ang/", content=enclosed)
        ...
        lang_scottish_gaelic_code = pp.nestedExpr("@gla\\", "@gla/", content=enclosed)
        ...
        language_codes = (
            lang_ancient_greek_code
            ^ lang_anglo_norman_code
            ^ lang_cornish_code
            ^ lang_english_code
            ^ lang_french_code
            ^ lang_german_code
            ^ lang_italian_code
            ^ lang_latin_code
            ^ lang_middle_cornish_code
            ^ lang_middle_high_german_code
            ^ lang_middle_low_german_code
            ^ lang_middle_welsh_code
            ^ lang_old_english_code
            ^ lang_portuguese_code
            ^ lang_scottish_gaelic_code
            ^ lang_spanish_code
            ^ lang_welsh_code
        )
        ...
        signed_mark_centre_code = pp.nestedExpr("@smc\\", "@smc/", content=enclosed)
        signed_mark_centre_code.setParseAction(self._pa_signed_mark_centre)
        signed_mark_right_code = pp.nestedExpr("@smr\\", "@smr/", content=enclosed)
        ...
        tab_start_code = pp.nestedExpr(pp.LineStart() + "@[", "@]", content=enclosed)
        ...
        paired_codes = (
            ...
            ^ line_group_code
            ^ list_code
            ^ right_marginale_code
            ^ signed_code
            ^ signed_centre_code
            ^ signed_right_code
            ^ signed_mark_code
            ^ signed_mark_centre_code
            ^ signed_mark_right_code
            ^ superscript_code
            ^ tab_start_code
            ^ title_code
        )
        ...
        record_heading_date_century = pp.Word(pp.nums, min=2, max=2).setResultsName(
            "century"
        ) + pp.Literal("th Century").setResultsName("label")
        record_heading_date_century.setParseAction(self._pa_record_heading_date_century)
        circa = pp.Literal("@it\\c@it/ ").setParseAction(self._pa_circa)
        slash_year = pp.Literal("/") + pp.Word(pp.nums, min=1, max=4)
        start_year = (
            pp.Optional(circa).setResultsName("circa")
            + year.setResultsName("year")
            + pp.Optional(slash_year).setResultsName("slash_year")
        )
        end_year = pp.Word(pp.nums, min=1, max=4).setResultsName(
            "end_year"
        ) + pp.Optional(slash_year).setResultsName("slash_end_year")
        record_heading_date_year = start_year + pp.Optional(pp.oneOf("- –") + end_year)
        record_heading_date_year.setParseAction(self._pa_record_heading_date_year)
        record_heading_date = (
            record_heading_date_century
            ^ record_heading_date_year
            ^ pp.Literal("Undated")
        )
        language_code = pp.oneOf(
            "ang cnx cor cym deu eng fra gla gmh gml grc ita lat por spa wlm " "xno"
        )
        record_heading_content = (
            record_heading_place
            - pp.Literal("!").suppress()
            - record_heading_date
            - pp.Literal("!").suppress()
            - language_code
        )
        record_heading = pp.nestedExpr("@h\\", "\\!", content=record_heading_content)
        ...
        transcription_section = (
            blank + transcription_heading - pp.OneOrMore(table ^ enclosed)
        )
        ...
        source_head = blank + pp.nestedExpr("@sh\\", "@sh/", content=rich_content)
        source_head.setParseAction(self._pa_source_data)
        source_location = blank + pp.nestedExpr("@sl\\", "@sl/", content=rich_content)
        source_location.setParseAction(self._pa_source_data)
        source_repository = blank + pp.nestedExpr("@sr\\", "@sr/", content=rich_content)
        source_repository.setParseAction(self._pa_source_data)
        source_shelfmark = blank + pp.nestedExpr("@ss\\", "@ss/", content=enclosed)
        source_shelfmark.setParseAction(self._pa_source_data)
        source_date = blank + pp.nestedExpr("@st\\", "@st/", content=rich_content)
        ...
        ms_doc_desc_contents = pp.Optional(ms_ed_desc) + ms_source_data - ms_tech_desc
        ms_doc_desc = pp.nestedExpr("@md\\", "@md/", content=ms_doc_desc_contents)
        ms_doc_desc = ms_doc_desc.setResultsName("doc_desc", listAllMatches=True)
        ...
        print_tech_desc = enclosed.copy().setParseAction(self._pa_print_tech_desc)
        print_doc_desc_contents = (
            pp.Optional(print_ed_desc) + print_source_data - print_tech_desc
        )
        print_doc_desc = pp.nestedExpr("@pd\\", "@pd/", content=print_doc_desc_contents)
        print_doc_desc = print_doc_desc.setResultsName("doc_desc", listAllMatches=True)
        print_doc_desc.setParseAction(self._pa_print_doc_desc)
        abbreviation = pp.nestedExpr("@ab\\", "@ab/", content=pp.Word(pp.alphanums))
        expansion = pp.nestedExpr("@ex\\", "@ex/", content=rich_content)
        county = pp.nestedExpr("@ct\\", "@ct/", content=content)
        place_code = blank + abbreviation + blank + expansion + blank + county + blank
        place_code.setParseAction(self._pa_place_code)
        place_codes = pp.nestedExpr("@pc\\", "@pc/", content=pp.OneOrMore(place_code))
        preamble = (
            (ms_doc_desc ^ print_doc_desc)
            - blank
            - pp.Optional(comment_code + blank)
            - place_codes
        )
        record = (
            blank
        ...
        ).setResultsName("record", listAllMatches=True)
        record.setParseAction(self._pa_record)
        return (
            pp.StringStart() + blank + preamble + pp.OneOrMore(record) + pp.StringEnd()
        )
    ...
    def _make_foreign(self, lang_code, toks):
        return [
            '<foreign xml:lang="{}">{}</foreign>'.format(lang_code, "".join(toks[0]))
        ]
    ...
    def _make_signed_mark(self, s, loc, toks, rend_value=None):
        ...
        if rend_value:
            rend = ' rend="{}"'.format(rend_value)
        return ['<seg type="signed_mark"{}>'.format(rend), "".join(toks[0]), "</seg>"]
    ...
    def _pa_damaged(self, s, loc, toks):
        return [
            '<damage><gap unit="chars" extent="{}" /></damage>'.format(len(toks[1]))
        ]
    ...
    def _pa_illegible(self, s, loc, toks):
        return ['<gap extent="{}" reason="illegible" unit="chars" />'.format(toks[1])]
    ...
    def _pa_left_marginale(self, s, loc, toks):
        return [
            '<note type="marginal" place="margin_left">',
            "".join(toks[0]),
            "</note>",
        ]
    ...
    def _pa_ms_doc_desc(self, s, loc, toks):
        if len(toks[0]) < 4:
            ...
            raise pp.ParseFatalException(
                "@md begins with an apostrophe. For reasons unknown, this causes a problem. Please remove that initial apostrophe, validate/convert, and then add it back (to the Word document and TEI)"
            )
...
{}{}
            ...
            source_code, ms_identifier, ed_desc, tech_desc
        )
...
<msName>{}</msName>
                ...
                settlement, repository, shelfmark, ms_name
            ),
        ]
...
{}{}
            ...
            source_code, ed_title, ed_desc, tech_desc
        )
    ...
    def _pa_print_tech_desc(self, s, loc, toks):
        return '<note type="techDesc"><p>{}</p></note>'.format("".join(toks).strip())
    ...
    def _pa_record_heading(self, s, loc, toks):
        ...
        record = '<seg ana="taxon:{code}">{code}</seg>'.format(code=self._source_code)
        return [language_code, "<head>{} {} {}</head>\n".format(place, date, record)]

    def _pa_record_heading_date_century(self, s, loc, toks):
        century = int(toks["century"])
        label = toks["label"]
        return [
            '<date from-iso="{}01" to-iso="{}00">{}{}</date>'.format(
    ...
    def _pa_right_marginale(self, s, loc, toks):
        return [
            '<note type="marginal" place="margin_right">',
            "".join(toks[0]),
            "</note>",
        ]
    ...
    def _pa_source_code(self, s, loc, toks):
        ...
        if code in self._source_codes:
            raise pp.ParseFatalException("Source code {} already used".format(code))
