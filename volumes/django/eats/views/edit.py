...
@user_passes_test(user_is_editor)
@add_topic_map
def show_duplicates (request, topic_map):
    dups = NameCache.objects.values('form').annotate(
        count=Count('form')).filter(count__gt=1).order_by('-count')
    real_dups = []
    for dup in dups:
        if NameCache.objects.filter(form=dup['form']).values('entity').annotate(
                entity_count=Count('entity')).count() > 1:
            real_dups.append(dup)
    context_data = {'dups': real_dups}
    return render(request, 'eats/edit/duplicates.html', context_data)
