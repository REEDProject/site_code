...
class AuthorityViewsTestCase(AdminViewTestCase):
    ...
    def test_authentication(self):
        ...
        for url_name, kwargs in url_data:
            ...
            self.assertRedirects(
                response, login_url, msg_prefix="Anonymous user to {}".format(url_name)
            )
        for url_name, kwargs in url_data:
    ...
    def test_authority_add_post_redirects(self):
        ...
        redirect_url = reverse(
            "eats-authority-change", kwargs={"topic_id": authority.get_id()}
        )
    ...
    def test_authority_change_get(self):
        url = reverse(
            "eats-authority-change", kwargs={"topic_id": self.authority.get_id()}
        )
        response = self.app.get(url, user="staff")
        self.assertEqual(response.status_code, 200)
    ...
    def test_authority_change_post(self):
        ...
        name_type = self.create_name_type("Test")
        script = self.create_script("Latin", "Latn", " ")
        user1 = self.create_django_user("user1", "user1@example.org", "password")
        editor1 = self.create_user(user1)
        user2 = self.create_django_user("user2", "user2@example.org", "password")
        editor2 = self.create_user(user2)
        form = self.app.get(url, user="staff").forms["infrastructure-change-form"]
        form["name"] = "Test1"
        form["calendars"] = [calendar1.get_id(), calendar2.get_id()]
        form["date_periods"] = [date_period.get_id()]
        form["date_types"] = [date_type.get_id()]
        ...
        self.assertTrue(
            entity_relationship_type in self.authority.get_entity_relationship_types()
        )
    ...
    def test_authority_illegal_remove(self):
        ...
        entity.create_entity_type_property_assertion(self.authority, entity_type1)
        url = reverse(
            "eats-authority-change", kwargs={"topic_id": self.authority.get_id()}
        )
        form = self.app.get(url, user="staff").forms["infrastructure-change-form"]
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            set(self.authority.get_entity_types()), set([entity_type1, entity_type2])
        )
        self.assertEqual(len(self.authority.get_calendars()), 0)
